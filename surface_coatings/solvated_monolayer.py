...
class SolvatedMonolayer(mb.Compound):
    ...
    def __init__(self, monolayer, solvent=H2O(), n_solvents=1000, solvent_box_height=5, seed=12345):
        ...
        surface_bounding_coords = ((xs[0], xs[-1]), (ys[0], ys[-1]), (zs[0], zs[-1]))
        ...
        box_of_solvent.translate([surface_bounding_coords[0][0],
...
class SolvatedDualMonolayer(mb.Compound):
    ...
    def __init__(self, dual_monolayer, solvent=H2O(), n_solvents=1000, seed=12345):
        ...
        for pos in bottom_monolayer["tiled_surface"].xyz:
            xs.append(pos[0]), ys.append(pos[1]), zs.append(pos[2])
        xs.sort(), ys.sort(), zs.sort()
        bottom_surface_coords = ((xs[0], xs[-1]), (ys[0], ys[-1]), (zs[0], zs[-1]))
        bottom_surface_box_lengths = bottom_monolayer["tiled_surface"].get_boundingbox().lengths
        ...
        solvent_box = [bottom_surface_box_lengths[0],
                       bottom_surface_box_lengths[1],
                       separation]
        box_of_solvent = mb.fill_box(compound=solvent,
                                     box=solvent_box,
                                     n_compounds=n_solvents)
        box_of_solvent.translate([bottom_surface_coords[0][0],
